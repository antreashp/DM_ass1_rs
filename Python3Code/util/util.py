...
def print_latex_statistics_clusters(dataset, cluster_col, input_cols, label_col):
    ...
    for l in label_cols:
        ...
        for cluster in clusters:
            print(len(dataset[dataset[l] == 1].index))
            print(' & ', "{0:.2f}".format((float(dataset.loc[dataset[cluster_col] == cluster, l].sum())/(len(dataset[dataset[l] == 1].index)+0.00000001) * 100)), '\%', end='')
        print('\\\\')
