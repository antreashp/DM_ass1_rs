...
class VisualizeDataset:
    ...
    def __init__(self, module_path='.py',user ='book'):
    ...
    def save(self, plot_obj, formats=('png',)): # 'svg'
        ...
        fig_name = f'figure_{self.plot_number}' + '_'+self.user
    ...
    def plot_dataset(self, data_table, columns, match='like', display='line'):
        ...
        for i in range(0, len(columns)):
            ...
            for x in range(len(min_values)):
                try:
                    min_v.append(float(min_values[x]))    
                except:
                    pass
                try:
                    max_v.append(float(max_values[x]))    
                except:
                    pass
            max_values= max_v
            min_values= min_v
