...
for user in range(1,36):
    filename = "..\my_dataset_final\\" + str(user) +'\\accelerometer.csv' 
    print(filename)
    ...
    instance_data = pd.read_csv(filename) #get the data
    # instance_id , year, month , date, hours,minutes,seconds = instance_id.split('_') # get info from file name
    # data['acc_z'] = data['acc_z'].map(lambda x: float(str(x).strip(';')))



    if instance_data.to_numpy().shape[0] == 0:
        continue    #skip if no data
    minn = instance_data['timestamp'].min()
    maxx = instance_data['timestamp'].max()
